...
std::string hresultText(HRESULT hr, const char *context)
{
    ...
    case AUDCLNT_E_DEVICE_INVALIDATED:
        hint = "The selected output device was unplugged, disabled, or changed by Windows.";
        break;
    case AUDCLNT_E_SERVICE_NOT_RUNNING:
        hint = "The Windows Audio service is not running.";
        break;
    case AUDCLNT_E_ENDPOINT_CREATE_FAILED:
        hint = "Windows could not open the selected output endpoint.";
        break;
    case AUDCLNT_E_DEVICE_IN_USE:
        hint = "The output device is already in exclusive use by another app.";
        break;
    case E_ACCESSDENIED:
        hint = "System audio capture was denied by Windows device policy.";
        break;
    default:
        break;
    }
...
std::string friendlyNameFor(ComPtr<IMMDevice> device)
{
    ...
    std::string result = "System audio";
    ...
    std::string &error)
{
    ...
    if (!isPcmFormat(format)) {
        error = "Unsupported system audio sample format: " + formatDescription(format);
        return false;
    }
    ...
    error = "Unsupported system audio PCM bit depth: " + std::to_string(bitsPerSample);
    return false;
}

} // namespace

WindowsSystemAudioCapture::WindowsSystemAudioCapture(const privacy::PrivacyManager &privacyManager)
    : m_privacyManager(privacyManager)
{
}

WindowsSystemAudioCapture::~WindowsSystemAudioCapture()
{
    stopSystemAudioCapture();
}

std::vector<AudioOutputDevice> WindowsSystemAudioCapture::listOutputDevices()
{
    ScopedCom com;
    if (!com.ok()) {
    ...
    if (FAILED(hr)) {
        setLastError(hresultText(hr, "Enumerating output devices"));
        return {};
    }
    ...
    if (devices.empty()) {
        setLastError("No active Windows output devices were found.");
    } else {
        setLastError("");
    }
    return devices;
...
bool WindowsSystemAudioCapture::selectOutputDevice(const std::string &deviceId)
{
...
std::string WindowsSystemAudioCapture::selectedOutputDeviceId() const
{
    std::lock_guard lock(m_mutex);
    return m_selectedDeviceId;
}

bool WindowsSystemAudioCapture::startSystemAudioCapture()
{
    ...
    const bool started = m_systemAudioActive;
    lock.unlock();
...
void WindowsSystemAudioCapture::stopSystemAudioCapture()
{
    {
        std::lock_guard lock(m_mutex);
...
double WindowsSystemAudioCapture::currentOutputLevel() const
{
    std::lock_guard lock(m_mutex);
    return m_levelMeter.level();
}

SystemAudioDiagnostics WindowsSystemAudioCapture::diagnostics() const
{
    std::lock_guard lock(m_mutex);
    auto diagnostics = m_diagnostics;
    diagnostics.captureActive = m_systemAudioActive;
...
std::string WindowsSystemAudioCapture::lastError() const
{
    std::lock_guard lock(m_mutex);
    return m_lastError;
}

void WindowsSystemAudioCapture::captureLoop()
{
    ScopedCom com;
    if (!com.ok()) {
    ...
    if (!device) {
        completeStart(false, "No Windows output device is available for loopback capture.");
        return;
    }
    ...
    if (FAILED(hr)) {
        completeStart(false, hresultText(hr, "Activating output audio client"));
        return;
    }
    ...
    if (FAILED(hr) || mixFormat == nullptr) {
        completeStart(false, hresultText(hr, "Reading output mix format"));
        return;
    }
    ...
    if (hr == S_FALSE && closestMatch != nullptr) {
        ...
        CoTaskMemFree(mixFormat);
        completeStart(false, hresultText(hr, "Checking output mix format support"));
        return;
    } else if (hr != S_OK) {
        ...
        CoTaskMemFree(mixFormat);
        completeStart(false, "Windows did not provide a supported shared-mode output format.");
        return;
    }
    if (closestMatch != nullptr) {
        CoTaskMemFree(closestMatch);
    ...
    if (!isSupportedCaptureFormat(mixFormat)) {
        const std::string error = "Unsupported system audio sample format: " + formatDescription(mixFormat);
        CoTaskMemFree(mixFormat);
    ...
    if (FAILED(hr)) {
        CoTaskMemFree(mixFormat);
        completeStart(false, hresultText(hr, "Initializing system audio loopback"));
        return;
    }
    ...
    if (captureEvent == nullptr) {
        CoTaskMemFree(mixFormat);
        completeStart(false, "Failed to create system audio capture event.");
        return;
    }
    ...
    if (FAILED(hr)) {
        ...
        CoTaskMemFree(mixFormat);
        completeStart(false, hresultText(hr, "Setting system audio event handle"));
        return;
    }
    ...
    if (FAILED(hr)) {
        CloseHandle(captureEvent);
        CoTaskMemFree(mixFormat);
        completeStart(false, hresultText(hr, "Opening system audio capture client"));
        return;
    }
    ...
    if (FAILED(hr)) {
        CloseHandle(captureEvent);
        CoTaskMemFree(mixFormat);
        completeStart(false, hresultText(hr, "Starting system audio loopback"));
        return;
    }
    ...
    while (!stopRequested() && !captureError) {
        ...
        if (waitResult != WAIT_OBJECT_0 && waitResult != WAIT_TIMEOUT) {
            setLastError("System audio capture wait failed.");
            break;
        }
        ...
        if (FAILED(hr)) {
            setLastError(hresultText(hr, "Reading system audio packet size"));
            break;
        }
        ...
        while (packetFrames > 0) {
            ...
            if (FAILED(hr)) {
                setLastError(hresultText(hr, "Reading system audio packet"));
                captureError = true;
                break;
            }
...
void WindowsSystemAudioCapture::completeStart(bool ok, const std::string &error)
{
    {
        std::lock_guard lock(m_mutex);
        m_systemAudioActive = ok;
...
bool WindowsSystemAudioCapture::stopRequested() const
{
    std::lock_guard lock(m_mutex);
    return m_stopRequested;
}

void WindowsSystemAudioCapture::setLastError(const std::string &error)
{
    std::lock_guard lock(m_mutex);
    m_lastError = error;
    m_diagnostics.lastError = error;
